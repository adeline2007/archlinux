...
# Базовое знание работы с консолью
Пишу это как шпору для человека, который этого не знает.

Вдруг полезно будет ещё кому-то

* cd папка   // переместиться директорию
* cd ..   //выйти на директорию выше (выйти из папки)
* pwd    // узнать, в какой вы директории
* ls    // увидеть какие файлы есть в директории
* mkdir имя_новой_директории  // создание директории
* reboot   // перезагрузка. Наше всё.
...
Красивое открытие кода (ну или просто текстовых файлов)
с оформлением. Явно красивее чем cat
...
// Записываем туда
preload = /путь_до изображения
wallpaper = , /путь до изображения
```

# А теперь лютый подгон от меня. **Кастомная загрузка и случайные обои!**

Суть: При запуске пк будет выбираться случайное изображение из папки, которую
вы укажете в коде и оно будет в качестве обоев. Для этого советую сделать папку
где-то wallpapers. После будет открываться консоль, выводиться ascii арт
и случайная преведственная надпись. Всё это ты сможешь настроить под себя!)

Тк мы устанавливали git clone от с этой страницы. То идём к той папке
(О, вдруг кому-то удобнее будет в начале глянуть с помощью dolphin. Нажимаем win+R и ищем его)

Но лучше в консоли)

Короче мы должны перейти к внутрь папки и узнать путь до неё (pwd)
Теперь идём в конфиг файл hyprland и добавляем базовую загрузку

`exec-once = python /адрес_до_файла_start_fetch.py`

О, если нету python

`yay -S python`   //Вроде...

Зайдем в start_fetch. Удобно в vscode.

Идём на 63 строку и должны указать нашу папку, где храняться обои.
Остальное в коде прокоментированно... Надеюсь вы питон знаете) Но по аналогии можно
всё будет настроить) Успехов!
